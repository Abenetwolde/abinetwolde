        ...
        <SectionWrapper id="experience" className="min-h-screen">
            ...
            <div className="w-fit mx-auto mt-6 p-2 bg-white dark:bg-grey-800 rounded-md flex gap-2 items-center">
                {['Experience', 'Education'].map((e, i) => (
                    <button key={i} onClick={() => handleTabClick(e)} className={`py-2 px-4 rounded-md transition-colors ${show === e ? 'bg-pink-600 text-white' : 'hover:bg-gray-100 hover:dark:bg-grey-900 text-black dark:text-white'}`}>{e}</button>
                ))
                }
            </div>
